...
test("the domain may not import built-ins, packages or other layers", () => {
  ...
  assert.equal(checkFile("src/domain/task.ts", 'import { z } from "zod";').length, 1);
  assert.equal(checkFile("src/domain/task.ts", 'import type { TaskRepository } from "../application/ports.ts";').length, 1);
  assert.deepEqual(checkFile("src/domain/task.ts", 'import { other } from "./other.ts";'), []);
});

test("the application may not reach adapters or the composition root", () => {
  const multiline = 'import {\n  MemoryTaskRepository,\n} from "../adapters/memory-task-repository.ts";';
  assert.equal(checkFile("src/application/task-service.ts", multiline).length, 1);
  assert.equal(checkFile("src/application/task-service.ts", 'const c = await import("../config.ts");').length, 1);
  assert.deepEqual(checkFile("src/application/task-service.ts", 'import { createTask } from "../domain/task.ts";'), []);
});

test("adapters may use built-ins and packages, but not the composition root", () => {
  assert.deepEqual(checkFile("src/adapters/http.ts", 'import { createServer } from "node:http";\nimport x from "some-package";'), []);
